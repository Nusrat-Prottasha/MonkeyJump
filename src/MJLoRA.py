# monkeyjump_lora.py
# MonkeyJump-LoRA: MoE-LoRA with routing + probability-weighted experts + jitter noise
...
# -----------------------------
# Masked LoRA Linear (One Expert per Linear)
# -----------------------------

class MonkeyJumpLinear(nn.Module):
    """
    LoRA adapter - ONE expert per linear layer.
    ...
    NOTE: LoRA weights (A, B) are kept in float32 for gradient precision during bfloat16 training.
    """
    ...
    def __init__(
        ...
        # FIX: LoRA weights in float32 for gradient precision during bfloat16 training
        self.A = nn.Parameter(torch.empty(rank, Din, device=base_device, dtype=torch.float32))
    ...
    def __repr__(self):
        ...
        return f"MonkeyJumpLinear[{mode}](in={self.in_features}, out={self.out_features}, rank={self.rank}, dtype=float32)"
...
def apply_monkeyjump(
    ...
    *,
    ...
    """
    Apply MonkeyJump-LoRA to selected blocks.

    MonkeyJump-LoRA is a Mixture-of-Experts LoRA with:
    ...
    Returns:
        Modified model with MonkeyJump-LoRA applied
        
    Note:
        LoRA weights (A, B) are kept in float32 regardless of model dtype to ensure
...
def get_lora_parameters(model: nn.Module) -> List[nn.Parameter]:
    """Get all LoRA A and B parameters."""
    params = []
...
def print_monkeyjump_summary(model: nn.Module):
    """Print a summary of MonkeyJump-LoRA configuration."""
    print("=" * 60)
    print("MonkeyJump-LoRA Summary")
    print("=" * 60)
    ...
    print(f"Trainable params: {counts['lora']:,}")
    print("=" * 60)
...
def check_lora_gradients(model: nn.Module):
    """
    Check gradient flow to LoRA weights. Call after loss.backward().
    ...
    for name, m in model.named_modules():
        if isinstance(m, MonkeyJumpLinear):
            if m.A.grad is not None and m.B.grad is not None:
                a_grad_norm = m.A.grad.norm().item()
                b_grad_norm = m.B.grad.norm().item()
                print(f"{name}:")
                print(f"  A grad norm: {a_grad_norm:.6f}, dtype: {m.A.dtype}")
                print(f"  B grad norm: {b_grad_norm:.6f}, dtype: {m.B.dtype}")
            else:
                has_a = m.A.grad is not None
                has_b = m.B.grad is not None
                print(f"{name}: A grad={has_a}, B grad={has_b}")
